...
handlers.index = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      'head.title' : 'Home',
      'head.description' : 'Most terrible pizza deliver to your dooer in 5mins',
      'body.class' : 'index'
    };
...
handlers.login = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      'head.title' : 'Log in',
      'head.description' : 'Log in to order your favourite pizza',
      'body.class' : 'login'
    };
...
// Log out
handlers.loggedOut = function(data,callback) {
  // Reject any request that is not a GET
  if (data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description' : 'You have already logged out. Hope not to see you again.',
...
handlers.signUp = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      'head.title' : 'Sign Up',
      'head.description' : 'Sign up to get your favourite pizza',
      'body.class' : 'signup'
    };
...
handlers.order = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      'head.title' : 'Order',
      'head.description' : 'Order your favourite pizza',
      'body.class' : 'order'
    };
...
handlers.cart = function(data,callback) {
  if (data.method =='get') {
    var templateData = {
      ...
      'head.description' : 'Your shopping cart',
      'body.class' : 'cart'
    }
    // Read in a template as a string
...
handlers.pay = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      'head.title' : 'Payment',
      'head.description' : 'Please settle your payment',
      'body.class' : 'pay'
    };

    // Read in a template as a string
    helpers.getTemplate('pay', templateData, function(err, str) {
      if (!err && str) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(str, templateData, function(err, str) {
          if (!err && str) {
...
handlers.paymentSuccess = function(data,callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description' : 'Payment is successful, pizza is on the way',
...
handlers._users.get = function(data, callback) {
  ...
  console.log('email', data.queryStringObject.email, email);
  if (email) {
    ...
    _data.read('tokens', token, function(err, tokenData) {
      if (!err && tokenData) {
        _data.delete('tokens', token, function (err) {
          if (!err) {
            _data.read('users', tokenData.email, function(err, usersData){
              if(!err && usersData) {
                delete usersData.token;
                usersData.orders="";
                _data.update('users', usersData.email, usersData, function(err) {
                  if (!err) {
                    callback(200)
                  } else {
                    callback(500, {'Error': 'Error deleting token from users record'})
                  }
                })
              }
            })
...
handlers._orders.get = function(data, callback) {
  ...
  var orderId = typeof(data.queryStringObject.orderId) == 'string' ? data.queryStringObject.orderId : false;
  if (orderId) {
    // Verify token
    var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
    if (token) {
